...
GitHub Desktop use webpack to compile, transpile and bundle its application
resources. The configuration files that webpack uses are found under `app/`:
...
Webpack also handles these steps:
 ...
 - source maps are generated to help with tracing JS errors to the TypeScript
   source files
...
Desktop uses `electron-builder` to generate these three packages:
 ...
 - `.rpm` package for various distributions
